...
func TestNoDeposits(t *testing.T) {
	...
	output := Derive(input)
	assert.Assert(t, output.L1BlockHash == input.Block.Hash())
	assert.Assert(t, output.L1BlockTime == input.Block.Time())
	assert.Assert(t, output.L1Random == Bytes32(input.Block.MixDigest()))
	assert.Assert(t, len(output.UserDepositTxs) == 0)
}

func assertDepositsEqual(t *testing.T, in *DepositTransaction, out *DepositTransaction) {
	assert.Assert(t, out.From == in.From)
	assert.Assert(t, out.To == in.To)
	assert.Assert(t, out.Value == in.Value)
	assert.Assert(t, out.GasLimit == in.GasLimit)
	assert.Assert(t, out.IsCreation == in.IsCreation)
	assert.Assert(t, bytes.Compare(out.Data, in.Data) == 0)
}

func TestSingleDeposit(t *testing.T) {
	input := GenerateInputItem(1, 0)
	output := Derive(input)
	assert.Assert(t, len(output.UserDepositTxs) == 1)
...
func TestManyDeposits(t *testing.T) {
	...
	output := Derive(input)
	assert.Assert(t, len(output.UserDepositTxs) == 3)
